...
雑談が盛り上がる **100の問いかけ** を内蔵した、くじ引きアプリです。
...
| ファイル | 役割 |
| --- | --- |
| `index.html` | アプリ本体（100問を内蔵。HTML1枚で完結。外部ライブラリ・API・ビルド不要） |
...
1. 上部のボタンでカテゴリを選びます（「すべて」なら100問すべてが対象）。
...
## 収録テーマ（10カテゴリ × 10問 ＝ 100問）

500問の候補から、**「全員が即答できる」「話が横に広がる」「派閥や意外性で反応が返る」**
を基準に各カテゴリ10問ずつ厳選しています。
...
問いかけの重複はありません。カテゴリをまたいだ内容の重なりも避けています。

絞り込む前の500問は git の履歴に残っています。戻したい場合はコミット
`同期機能を削除し、話題のくじ（500問）単体の静的サイトにする` の `index.html` を参照してください。
...
残り数の分母は `THEME_DATA` から自動で計算されるため、
1カテゴリあたりの件数が10でなくても、そのまま正しく表示されます。
